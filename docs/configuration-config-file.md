...
| `defaultEditableBy` | `owner`, `signed-in` or `anyone` | Set notes default permission (only applied on signed-in users). |
| `defaultViewableBy` | `owner`, `signed-in` or `anyone` | Set notes default permission (only applied on signed-in users). |
