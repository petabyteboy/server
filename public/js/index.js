...
function updateViewableBy (newViewableBy) {
  ...
  setActive(ui.infobar.permission.viewableBy.signedIn, newViewableBy === 'signed-in')
...
function updateEditableBy (newEditableBy) {
  ...
  setActive(ui.infobar.permission.editableBy.signedIn, newEditableBy === 'signed-in')
  setActive(ui.infobar.permission.editableBy.owner, newEditableBy === 'owner')
}

function canEdit () {
  if (editableBy === 'anyone') return true;
  if (editableBy === 'signed-in') return !!personalInfo.login;
...
socket.on('viewable-by', function (data) {
  updateViewableBy(data.viewableBy)
})
socket.on('editable-by', function (data) {
  updateEditableBy(data.permission)
})
